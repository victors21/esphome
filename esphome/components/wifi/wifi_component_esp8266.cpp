...
#define wifi_softap_set_dhcps_lease(lease) template<T> fix1(T lease){return true;}//getNonOSDhcpServer().set_dhcps_lease(lease)
#define wifi_softap_set_dhcps_lease_time(time) template<T>fix2(T time){return true;} //getNonOSDhcpServer().set_dhcps_lease_time(time)
#define wifi_softap_set_dhcps_offer_option(offer, mode) template<T,T1>fix3(T offer,T1 mode){return true;} //getNonOSDhcpServer().set_dhcps_offer_option(offer, mode)
#endif
}
